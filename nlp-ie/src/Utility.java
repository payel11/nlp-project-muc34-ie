...
public class Utility {
	...
	public static ArrayList <String> victimWordsNoun;
	public static ArrayList <String> victimWordsVerb;
	...
	static
	{
		...
		victimWordsNoun = new ArrayList<String> ();
		victimWordsVerb = new ArrayList<String> ();
		readVictims();
		readPlaces();
		...
		weapons.add("CAR-BOMB");//handle later

		determiners.add("a");
		determiners.add("the");

		victimWordsNoun.add("KIDNAPPING");victimWordsNoun.add("KILLING");victimWordsNoun.add("ASSASSINATION");
		victimWordsNoun.add("MURDER");victimWordsNoun.add("MURDERS");victimWordsNoun.add("MASSACRE") ;
	...
	public static void readVictims()
	{
		String curDir = System.getProperty("user.dir");
		Reader r = new Reader();
		ArrayList <String> lines = r.readFile1(curDir +"/dictionary/victims");

		int size = lines.size();
		System.out.println("no of words in victims directory:"+size);


		System.out.println("\n\t total "+size+" victim words found.");

		for(int i=0;i<size;i++)
		{
			victimWordsVerb.add(lines.get(i));
			System.out.println("\n\t added victim Word:"+victimWordsVerb.get(i));
		}
		size = victimWordsVerb.size();
	...
	public static boolean containsBombing(String sentence)
	{
		...
		{ 
			if(words[i].equalsIgnoreCase("bombs")|| words[i].equalsIgnoreCase("bomb")|| words[i].equalsIgnoreCase("bombed")
					...
					|| words[i].equalsIgnoreCase("explosion")||words[i].equalsIgnoreCase("explosives")||words[i].equalsIgnoreCase("CAR-BOMB")
			)
			{
				return true;
			}
	...
	public static boolean containsVictimWordNoun(String sentence)
	{
		String [] words =sentence.split(" ");
		for(int i=0;i<words.length;i++)
		{ 
			int size = victimWordsNoun.size();
			for(int j=0;j<size;j++)
			{
				if(words[i].equalsIgnoreCase(victimWordsNoun.get(j)))
				{
					return true;
				}
			}
	...
	public static boolean containsVictimWordVerb(String sentence)
	{
		String [] words =sentence.split(" ");
		for(int i=0;i<words.length;i++)
		{ 
			int size = victimWordsVerb.size();
			for(int j=0;j<size;j++)
			{
				if(words[i].equalsIgnoreCase(victimWordsVerb.get(j)))
				{
					return true;
				}
			}
	...
	public static String getMatchedVictim(String sentence, int listIndex)
	{
		String [] words =sentence.split(" ");
		if(listIndex == 1)
		{
	...
	public static boolean isPresent(String s, String w)
	{
		...
		{
			if(values[i].equalsIgnoreCase(w))
				return true;

			if(values[i].contains(w))
				return true;
		}
		return false;
	}

	public static boolean isNextWordPresentTense (String s,String w)
	{
		String []ss = s.split(" ");
		for(int i=0;i<ss.length;i++)
		{
			if(ss[i].equalsIgnoreCase(w) && i < (ss.length - 2))
			{
				String tmp = ss[i+1];
				System.out.println(" tmp:"+tmp);
				if(tmp.endsWith("S")||tmp.endsWith("ES"))
					return true;
			}
		}
		return false;
	...
	public static boolean isInterviewer (String s,String w)
	{
		String []ss = s.split("[ | \n]");//check this.
		for(int i=0;i<ss.length;i++)
		{
			...
			if(token.contains("[") && token.contains("]"))
			{
				...
				System.out.println("Changed the token to:"+token);
				if(w.contains(token))
				{
					System.out.println("interviewer found");
					return true;
				}
			}
		}
	...
	public static ArrayList<String> makeUnique(ArrayList <String> l)
	{
		int size = l.size();
		int i=0;
		while(i<l.size())
		{
			...
			while(j<l.size())
			{
				...
				if(w1.contains(w2)||w1.equalsIgnoreCase(w2))
				{
					l.remove(j);
				}
				else if(w2.contains(w1)||w2.equalsIgnoreCase(w1))
				{
					l.remove(i);
				}
				else
	...
	public static void main(String... args)
	{
		...
		//u.readPlaces();
		//u.readVictims();
		//System.out.println(isNextWordPresentTense("[ZAMORA] THE FACT THAT PRESIDENT CRISTIANI ACKNOWLEDGES THAT MEMBERS OF THE ARMY WERE RESPONSIBLE FOR THE MASSACRE IS A FIRST STEP AND DEFINITELY A GOOD THING, BUT IT IS ENTIRELY INSUFFICIENT UNLESS THE SUBSEQUENT STEPS ARE ALSO TAKEN.", "CRISTIANI"));
		System.out.println(isDoubtFul("GENERAL MIGUEL ALFREDO MAZA MARQUEZ DIRECTOR OF THE ADMINISTRATIVE DEPARTMENT OF SECURITY OF COLOMBIA (DAS SECRET POLICE) BLAMED NARCOMILITARY ORGANIZATIONS FOR A CAR-BOMB ATTACK FROM WHICH HE ESCAPED UNHARMED TODAY."));
	}
